...
//	plot->SetXAxis1( new wxPLTimeAxis( 4, 200 ) );
	...
	bar1->SetGroup( "group1", group );
	bar2->SetGroup( "group1", group );
	bar3->SetGroup( "group1", group );
...
public: TextLayoutFrame() : wxFrame( 0, -1, "Text layout", wxDefaultPosition, wxSize(750,550) )
	...
	void OnSave( wxCommandEvent & )
	{
		...
		wxString font( "C:\\Users\\arond\\Projects\\wex\\pdffonts\\ComputerModernSansSerif.ttf" );
		if ( !doc.AddFont( "CMSSR", wxEmptyString, font ) 
...
void TestSankeyPlot( wxWindow *parent )
{
	wxFrame *frame = new wxFrame(parent, wxID_ANY, 
		wxT("Sankey plots in \x01dc\x03AE\x03AA\x00C7\x00D6\x018C\x01dd"), 
		wxDefaultPosition, wxScaleSize(500, 800));
	wxPLPlotCtrl *plot = new wxPLPlotCtrl(frame, wxID_ANY, wxDefaultPosition, wxDefaultSize);
	//plot->SetBackgroundColour( *wxWHITE );
	plot->SetTitle(wxT("Example sankey loss diagram for PV"));
	plot->ShowGrid( false, false );
...
public:
	bool OnInit()
	{
		...
		{
			if ( 0 == ::wxFreeTypeLoadAllFontFiles( wexdir + "/pdffonts" ) )
				wxMessageBox("Could not add font dir: " + wexdir + "/pdffonts" );
			//if (!wxPLPlot::SetDefaultFontFamily( "CMU Serif Upright Italic" ) )
			//	wxMessageBox("Could not set default pdf font to CMU Serif Upright Italic" );
		}
