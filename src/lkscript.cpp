...
static void CreatePlot( wxPLPlot *plot, double *x, double *y, int len, double thick, wxColour &col, int type,
	const wxString &xlab, const wxString &ylab, const wxString &series,
	int xap, int yap, int ppos,
	double baseline, const wxString &stackon, const wxString &grouping, 
	const wxString &lnsty, const wxString &marker )
{
	if (len <= 0 ) return;
	...
	// update all bar plot groupings
	wxPLBarPlot *barplot = dynamic_cast<wxPLBarPlot*>(p);
	if ( !grouping.IsEmpty() && barplot )
	{
		std::vector<wxPLBarPlot*> barplotgroup;
		wxArrayString groupnames;
		size_t n = plot->GetPlotCount();
		for( size_t i=0;i<n;i++ )
		{
			if ( wxPLBarPlot *bar = dynamic_cast<wxPLBarPlot*>( plot->GetPlot(i) ) )
			{
				wxPLPlot::PlotPos barpos;
				plot->GetPlotPosition( bar, 0, 0, &barpos );
				if ( barpos == ppos && grouping == bar->GetGroupName() )
					barplotgroup.push_back( bar );
			}
		}

		barplotgroup.push_back( barplot );
		for( size_t i=0;i<barplotgroup.size();i++ )
			barplotgroup[i]->SetGroup( grouping, barplotgroup );
	}


	p->SetXDataLabel( xlab );
	p->SetYDataLabel( ylab );
	plot->AddPlot( p, (wxPLPlotCtrl::AxisPos)xap, (wxPLPlotCtrl::AxisPos) yap, (wxPLPlotCtrl::PlotPos) ppos );
	plot->Invalidate();

	if ( wxPLPlotCtrl *pc = dynamic_cast<wxPLPlotCtrl*>(plot) )
		pc->Refresh();
}
...
static wxPLPlot *GetPlotSurface( wxWindow *parent )
{
...
void wxLKSetPlotTarget( wxPLPlot *plot )
{
	s_curPlot = plot;
}

wxPLPlot *wxLKGetPlotTarget()
{
	return s_curPlot;
}
...
{
	LK_DOC("plot", "Creates an XY line, bar, horizontal bar, or scatter plot. "
		"Options include thick/size, type, color, xap=top/bottom, yap=left/right, ppos=top/bottom, "
		"xlabel, ylabel, series, baseline, "
		"grouping (for vbar plots), stackon, style.", "(array:x, array:y, table:options):void");
	
	wxPLPlot *plot = GetPlotSurface( 
		...
		&& a0.length() > 0 )
	{
		...
		int ppos = wxPLPlotCtrl::PLOT_TOP;
		wxString stackon, grouping;
		...
		if (cxt.arg_count() > 2 && cxt.arg(2).deref().type() == lk::vardata_t::HASH )
		{
			...
			if ( lk::vardata_t *arg = t.lookup("grouping") )
				grouping = arg->as_string();
			...
			if ( lk::vardata_t *arg = t.lookup("ppos"))
			{
				if ( arg->as_string() == "bottom") ppos = wxPLPlotCtrl::PLOT_BOTTOM;
			}
		...
		CreatePlot( plot, x, y, len, thick, col, type, xlab, ylab, 
			series, xap, yap, ppos,
			baseline, stackon, grouping, 
			lnsty, marker );
...
{
	LK_DOC("plotopt", 
		"Modifies the current plot properties like title, coarse, fine, legend, legendpos, legendborder, scale, font, window, border, space, showaxes, textface, textsize", 
		"(table:options):boolean");
	
	cxt.result().assign( 1.0 );


	wxPLPlot *plot = s_curPlot;
	if (!plot) {
		return;
	}
	...
	if ( lk::vardata_t *arg = cxt.arg(0).lookup("textface") )
		plot->SetFontFamily( arg->as_string() );
	
	if ( lk::vardata_t *arg = cxt.arg(0).lookup("textsize") )
		plot->SetFontSize( arg->as_number() );
...
void fcall_plotout( lk::invoke_t &cxt )
{
	...
	if ( wxPLPlotCtrl *plot = dynamic_cast<wxPLPlotCtrl*>( s_curPlot ) )
	{
...
{
	LK_DOC("axis", "Modifies axis properties (type=log,linear,time,label, label, labels[2D array of value,label for 'label' axis type], "
		"show, showlabel, min, max, ticklabels, ticksizes=[large,small]), decimals, thousep=t/f, ppos=top/bottom, on the current plot.", 
		"(string:axis name 'x1' 'y1' 'x2' 'y2', table:options):void");
	...
	if (lk::vardata_t *arg = cxt.arg(1).lookup("ppos"))
		if ( arg->as_string() == "bottom" ) ppos = wxPLPlot::PLOT_BOTTOM;

	if (axname == "x1") axis = plot->GetXAxis1();
	if (axname == "x2") axis = plot->GetXAxis2();
	if (axname == "y1") axis = plot->GetYAxis1(ppos);
	if (axname == "y2") axis = plot->GetYAxis2(ppos);
	...
	if ( lk::vardata_t *arg = cxt.arg(1).lookup("decimals") )
		decimals = arg->as_integer();

	if ( lk::vardata_t *arg = cxt.arg(1).lookup("thousep") )
		thousep = arg->as_boolean();

	if ( decimals != -1 || thousep != false )
	{
		axis->SetNumberFormat( decimals, thousep );
		mod = true;
	}
...
void fcall_sankey( lk::invoke_t &cxt )
{
	LK_DOC( "sankey", "Creates a Sankey loss/gain diagram from the provided data.", "(array:[ value,label,baseline=t/f],...]):none" );
	...
	wxPLSankeyPlot *sankey = new wxPLSankeyPlot;

	lk::vardata_t &x = cxt.arg(0);
	for( size_t i=0;i<x.length();i++ )
	{
		if ( x.index(i)->length() < 2 ) break;

		bool baseline = false;
		if ( x.index(i)->length() == 3 ) 
			baseline = x.index(i)->index(2)->as_boolean();

		sankey->Add( x.index(i)->index(0)->as_number(),
			x.index(i)->index(1)->as_string(),
			baseline );
	}
...
{
	LK_DOC( "sector", "Creates a sector plot from labeled data, with an optional inner section.  "
		"Options are: colors{array}, showvalues=t/f, calloutsize=#, holesize=frac, textspace=#, border=#, decimals=#, thousep=t/f.", 
		"(array:[[ value, label ]...], {array:[[ value, label ]...]}, table:options):none" );
	...
	if ( optidx < cxt.arg_count() )
	{
		...
		if ( lk::vardata_t *o = opts.lookup( "prefix" ) )
			prefix = o->as_string();
		if ( lk::vardata_t *o = opts.lookup( "decimals" ) )
			decimals = o->as_integer();
		if ( lk::vardata_t *o = opts.lookup( "thousep" ) )
			thousep = o->as_boolean();

		if ( !prefix.IsEmpty() || !suffix.IsEmpty() || decimals!=-1 || thousep )
			sector->SetFormat( wxNUMERIC_REAL, decimals, thousep, prefix, suffix );
	}
...
{
	LK_DOC( "contour", "Creates a contour plot from gridded x,y,z data."
		"Options are filled, colormap=jet/parula/grayscale/rainbow, "
		"reversecolors=t/f, scalelabels=['',''...], levels, min, max, "
		"decimals, thousep=t/f", "( matrix:x, matrix:y, matrix:z, { table:options } ):none" );
	...
	wxString cmap_name;
	bool filled = false;
	int decimals = -1; // default: generic %lg format
	bool thousep = false; // with no thousands separator
	size_t levels = 10;
	bool reversed = false;
	double min, max;
	...
	if ( cxt.arg_count() > 3 )
	{
		...
		if ( lk::vardata_t *o = opt.lookup( "decimals" ) )
			decimals = o->as_integer();
		if ( lk::vardata_t *o = opt.lookup( "thousep" ) )
			thousep = o->as_boolean();
	...
	cmap->SetFormat( decimals, thousep );
...
static wxImageArray s_frames;
	...
	if ( cxt.arg_count() == 0 )
	{
		...
		wxPLPlotCtrl *plot = dynamic_cast<wxPLPlotCtrl*>( GetPlotSurface( 
			(s_curToplevelParent!=0)
				? s_curToplevelParent 
				: GetCurrentTopLevelWindow() ) );
		if ( !plot ) return;
...
static wxPdfDocument *s_pdfDoc = 0;
static wxPLPdfOutputDevice *s_pdfOutDev = 0;
static wxPLFormattedOutputHelper *s_pdfHelper = 0;

static void DeletePdf()
{	
	if ( s_pdfDoc ) delete s_pdfDoc;
	if ( s_pdfOutDev ) delete s_pdfOutDev;
	if ( s_pdfHelper ) delete s_pdfHelper;
...
static void fcall_pdfpage( lk::invoke_t &cxt )
{
	LK_DOC( "pdfpage", "Add a page to the current PDF document. Options are: orientation='landscape','portrait', paper='a4','letter',[width,height], border", "([table:options]):none" );
	if ( !s_pdfDoc )
	{
		DeletePdf();

		s_pdfDoc = new wxPdfDocument( wxPORTRAIT, "pt", wxPAPER_A4 );
		s_pdfDoc->SetFont( "Helvetica", 0, 12 );
		s_pdfOutDev = new wxPLPdfOutputDevice( *s_pdfDoc );
		s_pdfOutDev->TextSize( 12.0 );
		s_pdfHelper = new wxPLFormattedOutputHelper(
				wxPLRealRect( 0.0, 0.0, s_pdfDoc->GetPageWidth(), s_pdfDoc->GetPageHeight() ),
				s_pdfOutDev );
	}

	double border = 0;
	wxPrintOrientation orient( wxPORTRAIT );
	wxPaperSize papersize = wxPAPER_A4;
	bool use_custom_size = false;
	wxRealPoint customsize;
	if ( cxt.arg_count() > 0 && cxt.arg(0).type() == lk::vardata_t::HASH )
	{
		if ( lk::vardata_t *x = cxt.arg(0).lookup( "orientation" ) )
			if ( x->as_string().Lower() == "landscape" )
				orient = wxLANDSCAPE;

		if ( lk::vardata_t *x = cxt.arg(0).lookup( "border" ) )
			border = x->as_number();

		if ( lk::vardata_t *x = cxt.arg(0).lookup( "paper" ) )
		{
			if ( x->type() == lk::vardata_t::VECTOR && x->length() > 1 )
			{
				customsize.x = x->index(0)->as_number();
				customsize.y = x->index(1)->as_number();
				use_custom_size = true;
			}
			else
			{
				wxString ssz = x->as_string().Lower();
				if ( "a3" == ssz ) papersize = wxPAPER_A3;
				else if ( "letter" == ssz ) papersize = wxPAPER_LETTER;
				else if ( "legal" == ssz ) papersize = wxPAPER_LEGAL;
				else papersize = wxPAPER_A4;
			}
		}
	}

	if ( use_custom_size )
		s_pdfDoc->AddPage( orient, customsize.x, customsize.y );
	else
		s_pdfDoc->AddPage( orient, papersize );
	
	s_pdfHelper->SetRect( wxPLRealRect( border, border, 
		s_pdfDoc->GetPageWidth()-border-border, 
		s_pdfDoc->GetPageHeight()-border-border ) );
	s_pdfHelper->MoveTo(0,0);
}

static void fcall_pdftext( lk::invoke_t &cxt )
{
	LK_DOC("pdftext", "Output text to the PDF page renderer.", "(...):none");
	if ( !s_pdfHelper ) return;
		
	wxString dat;
	for (size_t i=0;i<cxt.arg_count();i++)
		dat += cxt.arg(i).as_string();

	s_pdfHelper->RenderText( dat );
}

static void fcall_pdftextln( lk::invoke_t &cxt )
{
	LK_DOC("pdftextln", "Output text to the PDF page renderer, followed by a newline.", "(...):none");
	if (!s_pdfHelper ) return;
...
static void fcall_pdfimage( lk::invoke_t &cxt )
{
	LK_DOC( "pdfimage", "Output an image from a file to the PDF page render. Options include: width, height, dpi. If only one of width or height specified, aspect ratio is maintained.", 
		"(string:file, [ table:options ]):none");
	if (!s_pdfHelper) return;
	...
	if ( cxt.arg_count() > 1 && cxt.arg(1).type() == lk::vardata_t::HASH )
	{
		if ( lk::vardata_t *v = cxt.arg(1).lookup("width") )
			width = v->as_number();

		if ( lk::vardata_t *v = cxt.arg(1).lookup("height") )
			height = v->as_number();

		if ( lk::vardata_t *v = cxt.arg(1).lookup("dpi") )
			imgdpi = v->as_number();
	}
	...
	if( image.LoadFile( file ) )
	{
		...
		if ( width < 0 && height < 0 )
		{
			width = image.GetWidth() / imgdpi * 72;
			height = image.GetHeight() / imgdpi * 72;
		}

		s_pdfHelper->RenderImage( image, width, height );
	}
}

static void fcall_pdftable( lk::invoke_t &cxt )
{
	LK_DOC("pdftable", "Render a PDF table from the given 2x2 matrix of cells", "(matrix:cell data):none");
	if (!s_pdfHelper) return;
	...
	if (nrows > 0 && ncols > 0)
	{
		wxMatrix<wxString> table(nrows, ncols, wxEmptyString);
		for (size_t r = 0;r<nrows;r++)
		{
			for (size_t c = 0;c<ncols;c++)
			{
				...
				if (row->type() == lk::vardata_t::VECTOR
					&& c<row->length() )
					table(r,c) = row->index(c)->as_string();
			}
		}

		s_pdfHelper->RenderTable( table );
	}
}


static wxPLPlot *s_pdfPlot = 0;

static void fcall_pdfplotinit( lk::invoke_t &cxt )
{
	LK_DOC( "pdfplotinit", "Initialize a fully configurable plot surface on the PDF page.", "(none):none" );
	if ( !s_pdfDoc ) return;

	if ( s_pdfPlot ) delete s_pdfPlot;
	s_pdfPlot = new wxPLPlot;
	wxLKSetPlotTarget( s_pdfPlot );
}

static void fcall_pdfplotdraw( lk::invoke_t &cxt )
{
	LK_DOC( "pdfplotdraw", "Finalize and render the plot to the PDF document in the given area.", "(number:x, number:y, number:width, number:height):none" );

	if ( !s_pdfPlot || !s_pdfOutDev ) return;

	wxPLRealRect rect( cxt.arg(0).as_number(),
		cxt.arg(1).as_number(),
		cxt.arg(2).as_number(),
		cxt.arg(3).as_number() );
	s_pdfPlot->Render( *s_pdfOutDev, rect );

	delete s_pdfPlot;
	s_pdfPlot = 0;
}

static void fcall_pdfbar( lk::invoke_t &cxt )
{
	LK_DOC("pdfbar", 
	...
	double width = 300.0; // pt
	double height = 200.0; // pt
	int decimals = 2;
	wxColour color(211,211,211);
	...
	s_pdfHelper->RenderBarGraph( values, labels, 
		xlabel, ylabel, title, 
		show_values, width, height, 
		decimals, color, show_yaxis_ticks, ticks_format );
}

static void fcall_pdforigin( lk::invoke_t &cxt )
{
	LK_DOC( "pdforigin", "Set the origin position and bounding rectangle for text layout and rendering.", "(number: x, number: y, number:width, number:height ):none" );
	if ( !s_pdfHelper ) return;
	s_pdfHelper->SetRect( wxPLRealRect(
		cxt.arg(0).as_number(),
		cxt.arg(1).as_number(),
		cxt.arg(2).as_number(),
		cxt.arg(3).as_number() ) );
...
static void fcall_pdfmoveto( lk::invoke_t &cxt )
{
	LK_DOC("pdfmoveto", "Moves the current PDF page output cursor to the x y position specified.", "(real:x, real:y):none");
	if (!s_pdfHelper) return;
	s_pdfHelper->MoveTo( (float)cxt.arg(0).as_number(), 
		(float)cxt.arg(1).as_number() );
}

static void fcall_pdflineto( lk::invoke_t &cxt )
{
	LK_DOC("pdflineto", "Draw a line from the current position to the new coordinates specified using the current style.  Updates the current position also.", "(real:x, real:y):none");
	if (!s_pdfHelper) return;

	s_pdfHelper->LineTo( (float)cxt.arg(0).as_number(), (float)cxt.arg(1).as_number() );
}

static void fcall_pdfcursor( lk::invoke_t &cxt )
{
	LK_DOC("pdfcursor", "Retrieves the current x y position of the output cursor.", "(none):table");
	if (!s_pdfHelper) return;

	double x=0, y=0;
	s_pdfHelper->GetCursorPos( &x, &y );
	cxt.result().empty_hash();
	cxt.result().hash_item( "x", (double)x );
	cxt.result().hash_item( "y", (double)y );
}

static void fcall_pdfmeasure( lk::invoke_t &cxt )
{
	LK_DOC("pdfmeasure", "Calculates the width and height in inches of a piece of text in the current font.", "(string:text):table");
	if (!s_pdfHelper) return;

	double x=0, y=0;
	s_pdfHelper->Measure(cxt.arg(0).as_string(), &x, &y );
	cxt.result().empty_hash();
	cxt.result().hash_item( "width", (double)x );
	cxt.result().hash_item( "height", (double)y );
}

static void fcall_pdfstyle( lk::invoke_t &cxt )
{
	LK_DOC("pdfstyle", "Change the current formatting style. Faces: fixed,sanserif,serif Align:left,center,right Line Styles:solid,dotted", 
		"(table:options including face, size, bold, italic, align, color, line_style, line_width, wrap_text, "
		"header_size,header_align,header_bold,header_color,cell_align,grid_lines,header_line,col_sizes,row_sizes,table_border"
		"):none");
	
	if (!s_pdfHelper) return;

	/* text properties */
	wxString face( wxFT_FONT_DEFAULT_FAMILY );
	int size=12, align=wxLEFT;
	bool bold=false, ital=false;
	wxColour colour = *wxBLACK;
	double line_width = 0.5; // points
	wxPLOutputDevice::Style line_style = wxPLOutputDevice::SOLID;
	bool wraptext = false;

	s_pdfHelper->Style( &face, &size, &colour, &bold, &ital, &align, &line_width, &line_style, &wraptext );

	/* table properties */
	int hdrSize, hdrAlign;
	wxString hdrFace;
	bool hdrBold;
	wxColour hdrColour;
	int cellAlign;
	bool gridLines, hdrLine, tabBorder;
	std::vector<double> rowSizes, colSizes;

	s_pdfHelper->TableStyle( &hdrSize, &hdrFace, &hdrAlign, &hdrBold, &hdrColour,
	...
	if ((v=tab.lookup("line_style")))
	{
		...
		ssty.Lower();
		if (ssty == "solid") line_style = wxPLOutputDevice::SOLID;
		else if (ssty == "dotted") line_style = wxPLOutputDevice::DOT;
		else throw lk::error_t("invalid line style specification.  allowed are 'solid', 'dotted'");
	}

	if ((v=tab.lookup("wrap_text")))
		wraptext = v->as_boolean();
	...
	s_pdfHelper->TableStyle( hdrSize, hdrFace, hdrAlign, hdrBold,
		hdrColour, hdrLine, cellAlign, gridLines, rowSizes, colSizes,
		tabBorder);
}

void fcall_pdfwrite( lk::invoke_t &cxt )
{
	LK_DOC( "pdfwrite", "Finalize and write PDF to disk.", "(string:file):boolean" );

	cxt.result().assign( 0.0 );

	if ( !s_pdfDoc ) return;
	const wxMemoryOutputStream &data = s_pdfDoc->CloseAndGetBuffer();

	wxString pdf_file( cxt.arg(0).as_string() );
	wxFileOutputStream fp( pdf_file );
	if ( fp.IsOk())
	{
		wxMemoryInputStream tmpis( data );
		fp.Write( tmpis );
		cxt.result().assign( fp.Close() ? 1.0 : 0.0 );
	}
...
{
	LK_DOC( "curl", "Issue a synchronous HTTP/HTTPS request.  Options are: 'post', 'header', 'message', 'file'.  If 'file' is specified, data is downloaded to that file and true/false is returned. Otherwise, the retrieved data is returned as a string.  HTTP 'header' can be a single string or an array of multiple headers.", 
	...
	if ( cxt.arg_count() > 1 && cxt.arg(1).type() == lk::vardata_t::HASH )
	{
		...
		if ( lk::vardata_t *x = opt.lookup( "header" ) )
		{
			if ( x->type() == lk::vardata_t::VECTOR )
			{
				...
				curl.AddHttpHeader( x->as_string() );
		}
