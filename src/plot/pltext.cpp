	...
	: m_bounds(0, 0)
{
	...
	for ( size_t i=0;i<m_lines.size(); i++ )
	{
		for ( size_t j=0;j<m_lines[i].size(); j++ )
		{
			...
			double width, height;
			dc.TextSize( tp.state == text_piece::NORMAL ? fontPoints : fontPoints-FontPointAdjust );
...
{
	if ( ifnt < 0 || ifnt >= ft_faces.size() 
		|| 0 == ft_faces[ifnt].builtin_data
		|| 0 == ft_faces[ifnt].builtin_len )
		return false;

	if ( FILE *f = fopen( (const char*)file.c_str(), "wb" ) )
	{
		fwrite( ft_faces[ifnt].builtin_data,
			1, ft_faces[ifnt].builtin_len, f );
...
static bool check_freetype_init()
{
	if ( ft_library == 0 )
	{
		...
		if ( inflate_builtin_fonts() )
		{
			...
			{
				unsigned char *inflated_data = &(*gs_inflatedFontData[i])[0];
				size_t inflated_len = gs_inflatedFontData[i]->size();
				FT_Face face;
				FT_Error err = FT_New_Memory_Face( ft_library, 
					inflated_data, inflated_len,
					0, &face );
				...
				if ( 0 == err )
				{
					...
					fi.family = builtinfonts[i].face;
					fi.file = wxEmptyString;
					fi.builtin_data = inflated_data;
					fi.builtin_len = inflated_len;
					fi.bold = &builtinfonts[i].bold;
					fi.italic = &builtinfonts[i].italic;
...
int wxFreeTypeLoadAllFontFiles( const wxString &path )
{
	wxString dir;
	if ( path.IsEmpty() )
		dir = wxPathOnly(wxStandardPaths::Get().GetExecutablePath()) + "/pdffonts";
	else
		dir = path;
	...
	for( size_t k=0;k<files.size();k++ )
	{
		...
		if ( ext == "ttf" || ext == "otf" )
		{
			int face = wxFreeTypeLoadFontFile( files[k] );
			if ( face >= 0 )
				n++;
		}
	}
...
int wxFreeTypeLoadFontFile( const wxString &font_file )
{
	...
	if ( 0 == err )
	{
		...
		fi.family = face->family_name ? wxString(face->family_name) : wxString("Default");
		fi.bold = face->style_flags & FT_STYLE_FLAG_BOLD;
		fi.italic = face->style_flags & FT_STYLE_FLAG_ITALIC;		
		fi.file = font_file;
		fi.builtin_data = 0;
		fi.builtin_len = 0;
		ft_faces.push_back( fi );
...
wxArrayString wxFreeTypeListFontFaces()
{
	...
	wxArrayString list;
	for( size_t i=0;i<ft_faces.size();i++ )
		list.Add( ft_faces[i].family );
	return list;
}

wxString wxFreeTypeFontFamily( int fnt )
{
	if ( fnt >= 0 && fnt < (int) ft_faces.size() )
		return ft_faces[fnt].family;
	else
		return wxEmptyString;
}
...
unsigned char *wxFreeTypeFontData( int ifnt, size_t *len )
{
	if ( ifnt >= 0 && ifnt < (int) ft_faces.size() )
	{
		*len = ft_faces[ifnt].builtin_len;
		return ft_faces[ifnt].builtin_data;
	}
...
{
	check_freetype_init();
	std::vector<int> ifnts;
	wxString ll(family.Lower());
	for( size_t i=0;i<ft_faces.size();i++ )
		if ( ft_faces[i].family.Lower() == ll )
			ifnts.push_back( (int) i );

	if ( 0 == ifnts.size() )
		return 0; // return the default built in font first

	// perfect match
	for( size_t i=0;i<ifnts.size();i++ )
		if ( ft_faces[ ifnts[i] ].bold == bold && ft_faces[ ifnts[i] ].italic == italic )
			return ifnts[i];

	// partial match for bold
	for( size_t i=0;i<ifnts.size();i++ )
		if ( ft_faces[ ifnts[i] ].bold == bold )
			return ifnts[i];

	// partial match for italic
	for( size_t i=0;i<ifnts.size();i++ )
		if ( ft_faces[ ifnts[i] ].italic == italic )
			return ifnts[i];
...
wxImage wxFreeTypeDraw( wxRealPoint *offset, int ifnt, double points, unsigned int dpi, const wxString &text, const wxColour &c, double angle )
{
	...
	wxRealPoint size( wxFreeTypeMeasure( ifnt, points, dpi, text ) );
...
#define OVERFLOW_SPACE 1.0
	// create image surface of appropriate size, with an extra pixel for a little overflow space
	wxSize bounds( wxRound(fabs(max.x-min.x)+OVERFLOW_SPACE), wxRound(fabs(max.y-min.y)+OVERFLOW_SPACE) );
	wxImage img( bounds, false );
	
	// find offset coordinate for top-left placement
	*offset = wxRealPoint( -min.x, max.y + ((angle==0.0)?0.0:OVERFLOW_SPACE) ); // if rotated, offset y a little into overflow space
...
wxRealPoint wxFreeTypeMeasure( int fnt, double points, unsigned int dpi, const wxString &text )
{
	...
	for( wxString::const_iterator it = text.begin(); it != text.end(); ++it )
	{
		...
		pen_x += ((double)face->glyph->advance.x) / 64.0;

		// save current glyph for next kerning
		previous = glyph_index;
	}
	
	return wxRealPoint( fabs(pen_x), 
...
void wxFreeTypeDemo::OnPaint( wxPaintEvent & )
{
	...
	wxString demotext( text + " (" + wxFreeTypeFontFamily( Face(1) ) + ")" );
	wxRealPoint bbox = wxFreeTypeMeasure( Face(1), 14, dpi, demotext );
	wxFreeTypeDraw( &img, true,  wxPoint(0,0), Face(1), 14,     dpi, demotext, *wxBLACK );
	wxFreeTypeDraw( &img, false, wxPoint(10,200),Face(2), 12,   dpi, text + " (" + wxFreeTypeFontFamily(Face(2)) + ")", *wxRED, 45.0 );
	wxFreeTypeDraw( &img, false, wxPoint(10,200),Face(1), 14,   dpi, text + " (" + wxFreeTypeFontFamily(Face(2)) + ")", *wxBLACK, -45.0 );
	...
	double dist = wxFreeTypeMeasure( Face(2), 12, dpi, "text to rotate").x;
	for( double angle = 0;angle<=270;angle+=22.5 )
	{
		wxPoint vec( (int)(dist*cos(angle*M_PI/180)),(int)(dist*sin(-angle*M_PI/180)) );
	...
	{
		wxString thetext( wxFreeTypeFontFamily(i) + ": The quick brown fox jumped..." );
		wxRealPoint sz = wxFreeTypeMeasure( i, 14, dpi, thetext );
	...
	dc.SetBrush( *wxTRANSPARENT_BRUSH );
	dc.SetPen( *wxLIGHT_GREY_PEN );
	dc.DrawRectangle( 0, 0, (int)bbox.x, (int)bbox.y );
