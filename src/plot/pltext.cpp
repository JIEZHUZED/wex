	...
	: wxWindow( parent, wxID_ANY )
{
	...
	int ypos = 36;
	wxArrayString builtin( wxFreeTypeListFonts() );
	for( size_t i=0;i< builtin.size();i++ )
	{
		faces.push_back( wxFreeTypeFindFont( builtin[i] ) );
		
		doc.SetFont( "Helvetica", 0, 8 );
		
		wxString text( builtin[i] + ": The quick brown fox...");

		bool bold = false, italic = false;
		wxFreeTypeFontStyle( faces.back(), &bold, &italic );

		int pdfstyle = wxPDF_FONTSTYLE_REGULAR;
		if ( bold ) pdfstyle |= wxPDF_FONTSTYLE_BOLD;
		if ( italic ) pdfstyle |= wxPDF_FONTSTYLE_ITALIC;

		if ( doc.SetFont( wxFreeTypeFontName( faces.back() ), pdfstyle, 14 ) )
			doc.Text( 36, ypos, text );
		else
		{
			doc.SetFont( "Helvetica", 0, 8 );
			doc.Text( 36, ypos, "style error with font: " + builtin[i] );
		}
	...
	wxString dir;
	wxGetEnv( "WEXDIR", &dir );
	wxString pdffile( dir + "/pdffonts/demo.pdf" );
