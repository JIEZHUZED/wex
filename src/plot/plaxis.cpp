...
void wxPLAxis::Init()
{
	...
	m_decimals = -1;
	m_thouSep = false;
	m_preText = m_postText = wxEmptyString;
}
...
void wxPLAxis::SetNumberFormat( int deci, bool thousep, const wxString &pre, const wxString &post )
{
	m_decimals = deci;
	m_thouSep = thousep;
	m_preText = pre;
...
wxString wxPLAxis::Format( double value )
{
	return wxNumericFormat( value, wxNUMERIC_REAL, m_decimals, m_thouSep, m_preText, m_postText );
}
...
void wxPLLinearAxis::GetAxisTicks( double phys_min, double phys_max, std::vector<TickData> &list )
{
	...
	for ( size_t i=0; i<largeticks.size(); i++ )
	{
		...
		if ( fabs(labelNumber) < 0.000000000000001 )
			labelNumber = 0.0;
		list.push_back( TickData( labelNumber, Format(labelNumber), TickData::LARGE ) );
	}
