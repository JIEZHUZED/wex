	...
	axis_layout( int ap, wxPLOutputDevice &dc, wxPLAxis *axis, double phys_min, double phys_max )
	{
		...
		if ( ap == X_BOTTOM || ap == X_TOP )
		{
			...
			if ( labeledTicks.size() > 1 )
			{
				...
				{
					double xphys = axis->WorldToPhysical( labeledTicks[j]->world, phys_min, phys_max );
					double width = labeledTicks[j]->text.Width();
					...
					// does it overlap with the label to the left?
					if ( j > 0 )
					{
						double xphys_left = axis->WorldToPhysical( labeledTicks[j-1]->world, phys_min, phys_max );
						double width_left = labeledTicks[j-1]->text.Width();
						
						rotate = xphys - width/2  < xphys_left + width_left/2;
						//wxLogDebug( "j=%d TO LEFT: %lg + %lg < %lg - %lg ==> rotate ? %d", (int)j, xphys, width/2, xphys_left, width_left/2, rotate?1:0 );
						if ( rotate )
							break;
					}
					
					// does it overlap with the label to the right?
					if ( j < labeledTicks.size() - 1 )
					{
						double xphys_right = axis->WorldToPhysical( labeledTicks[j+1]->world, phys_min, phys_max );
						double width_right = labeledTicks[j+1]->text.Width();

						rotate = xphys + width/2 > xphys_right - width_right/2;
						//wxLogDebug( "j=%d TO RIGHT: %lg + %lg < %lg - %lg ==> rotate ? %d", (int)j, xphys, width/2, xphys_right, width_right/2, rotate?1:0 );
						if ( rotate )
							break;
					}
				...
				if ( rotate )
				{
					...
					// recalculate bound height to rotated text of largest tick label
					m_bounds.y = labeledTicks[maxindex]->text.Height()
						+ fabs( labeledTicks[maxindex]->text.Width() * sin( M_PI/180*textAngle ) );
				}
			}
		}
		else
	...
	void render( wxPLOutputDevice &dc, double ordinate, wxPLAxis *axis, double phys_min, double phys_max, double ordinate_opposite = -1 )
	{
		...
		for ( size_t i=0;i<m_tickList.size(); i++ )
		{
			...
			if ( ti.text.Width() > 0 )
			{
				...
				//wxLogDebug("-->tick {%d} render (%lg,%lg) [%lg %lg]", (int)i, text_x, text_y, ti.text.Width(), ti.text.Height() );
			}
		}
...
static wxString s_defaultFontFamily;
...
wxPLPlot::wxPLPlot()
{
	...
	m_textFont = wxFT_FONT_DEFAULT_FAMILY;

	if ( !s_defaultFontFamily.IsEmpty() )
		m_textFont = s_defaultFontFamily;
...
bool wxPLPlot::SetDefaultFontFamily( const wxString &s )
{
	if ( wxFreeTypeListFontFaces().Index( s ) >= 0 )
	{
		s_defaultFontFamily = s;
		return true;
	}
	else
		return false;
...
{
#define NORMAL_FONT(dc)  dc.TextAdjust( 0 )
#define TITLE_FONT(dc)   dc.TextAdjust( +1 )
#define LEGEND_FONT(dc)  dc.TextAdjust( -1 )
#define AXIS_FONT(dc)    dc.TextAdjust( 0 )

	NORMAL_FONT(dc);
	dc.TextFace( m_textFont, false, false );
	dc.TextSize( m_textSize );
	...
	// note: first calculate Y axis boxes, so that box.x dimensions 
	// are reduced before calculating x axis layout and text rotations
	...
	// now that we have a better estimate of box width, calculate the x axis text layouts and any rotations
...
bool wxPLPlot::RenderPdf( const wxString &file, double width, double height )
{
	...
	doc.SetFont( "Helvetica", wxPDF_FONTSTYLE_REGULAR, m_textSize );
