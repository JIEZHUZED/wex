...
void wxPLColourMap::Copy( const wxPLColourMap &cpy )
{
	m_min = cpy.m_min;
	m_max = cpy.m_max;
	m_decimals = cpy.m_decimals;
	m_thousep = cpy.m_thousep;
...
void wxPLColourMap::SetFormat( int deci, bool thousep )
{
	m_decimals = deci;
	m_thousep = thousep;
	InvalidateBestSize();
...
wxRealPoint wxPLColourMap::CalculateBestSize( wxPLOutputDevice &dc )
{
	...
	else
	{
		for (int i=0; i<11; i++)
		{
			dc.Measure( wxNumericFormat( m_min+i*step, wxNUMERIC_REAL, m_decimals, m_thousep, wxEmptyString, wxEmptyString), &width, &height );
			if (width > maxWidth)
				maxWidth = width;
		}
	}
...
void wxPLColourMap::Render( wxPLOutputDevice &dc, const wxPLRealRect &geom)
{
	...
	if ( m_labels.size() < 2 )
	{
		...
		for (size_t i=0; i<11; i++)
			dc.Text( wxNumericFormat( m_min+i*step, wxNUMERIC_REAL, m_decimals, m_thousep, wxEmptyString, wxEmptyString), 
