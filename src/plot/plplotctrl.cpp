...
wxBitmap wxPLPlotCtrl::GetBitmap( int width, int height )
{
		...
		|| (m_includeLegendOnExport && !legend_shown ))
	{
		if ( m_includeLegendOnExport && !legend_shown )
		{
			...
			wxPLGraphicsOutputDevice odev( gc );
			odev.SetScale( wxGetScreenHDScale() );
			odev.TextSize( 12 );
...
void wxPLPlotCtrl::Render( wxGraphicsContext &gc, wxRect geom )
{
	double fontpoints = GetFontSize();
