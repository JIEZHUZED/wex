...
static bool is_valid_char( wxNumericMode mode, wxUniChar c, bool additional, wxUniChar c1 )
{
	...
	else return wxIsdigit(c) || c == '-' || c=='+' || c == '.' || c == 'e' || c == 'E' || c == 'a' || c == 'A' || c == 'n' || c == 'N' || (additional && c == c1);
}
...
void wxNumericCtrl::Translate()
{
	...
	{
		strval.MakeLower();
		if ( strval.Find( "nan" ) != wxNOT_FOUND 
				&& m_preText.Find("nan")==wxNOT_FOUND 
				&& m_postText.Find("nan")==wxNOT_FOUND)
			SetValue( std::numeric_limits<double>::quiet_NaN() );
		else if ( strval.Find( "inf" ) != wxNOT_FOUND 
				&& m_preText.Find("inf")==wxNOT_FOUND 
				&& m_postText.Find("inf")==wxNOT_FOUND )
			SetValue( std::numeric_limits<double>::infinity() );
		else
