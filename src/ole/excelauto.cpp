...
wxExcelAutomation::wxExcelAutomation()
{
...
bool wxExcelAutomation::StartExcel()
{
...
bool wxExcelAutomation::AttachExcel(bool start_if_fail)
{
	if (m_pdispExcelApp) delete m_pdispExcelApp;
...
bool wxExcelAutomation::QuitExcel()
{
	m_errStr = "";
...
bool wxExcelAutomation::Show(bool b)
{
	if (m_pdispExcelApp)
...
bool wxExcelAutomation::OpenFile(const wxString &fn)
{
	if (!m_pdispExcelApp)
	{
		m_errStr = "Excel OLE not initialized.";
...
bool wxExcelAutomation::WorkbookCount(int &count)
{
	count = 0;
...
bool wxExcelAutomation::SetWorkbook(int idx)
{
...
bool wxExcelAutomation::CloseWorkbookNoSave()
{
	if (!m_pdispExcelApp)
	{
		m_errStr = "Excel OLE not initialized.";
...
bool wxExcelAutomation::NewWorkbook()
{
	if (!m_pdispExcelApp)
	{
		m_errStr = "Excel OLE not initialized.";
...
bool wxExcelAutomation::AddWorksheet()
{
	if (!m_pdispWorkbook)
	{
...
bool wxExcelAutomation::SetWorksheetName(const wxString &name)
{
	if (!m_pdispWorksheet)
	{
...
bool wxExcelAutomation::PasteNewWorksheet(const wxString &name, const wxString &paste_text, 
									 bool std_formatting, bool in_current_worksheet)
{
	if (!in_current_worksheet && !AddWorksheet()) return false;
	if (!SetWorksheetName(name)) return false;

	wxTheClipboard->SetData( new wxTextDataObject( paste_text ) );
...
bool wxExcelAutomation::AutoFitColumns()
{
	if (m_pdispWorksheet)
	{
...
bool wxExcelAutomation::SetRowColBold(int rowcol, bool bold)
{
...
bool wxExcelAutomation::SetSelectedCellsFontSize(int sz)
{
	if (!m_pdispWorksheet) return false;
...
bool wxExcelAutomation::GetCellValue(int row, int col, wxString &val)
{
	if (!m_pdispWorksheet)
	{
		m_errStr = "No active worksheet, cannot set cell";
...
bool wxExcelAutomation::SetRangeValue(const wxString &range, const wxString &val)
{
	if (!m_pdispWorksheet)
	{
		m_errStr = "No active worksheet, cannot set cell";
...
bool wxExcelAutomation::SetNamedRangeArray(const wxString &namedRange, wxArrayString &val, bool as_row )
{
...
bool wxExcelAutomation::SetNamedRangeValue(const wxString &namedRange, const wxString &val)
{
...
bool wxExcelAutomation::GetNamedRangeValue(const wxString &namedRange, wxString &val)
{
...
bool wxExcelAutomation::GetRangeValue(const wxString &range, wxString &val)
{
	if (!m_pdispWorksheet)
	{
		m_errStr = "No active worksheet, cannot set cell";
...
wxString wxExcelAutomation::GetLastError()
{
	return m_errStr;
}


void wxExcelAutomation::ClearArgs()
{
