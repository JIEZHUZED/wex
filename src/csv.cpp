...
bool wxCSVData::Read( wxInputStream &in, int maxln )
{
/*
TEST CSV FILE: (r/w)
...
*/
	...
	while ( !txt.GetInputStream().Eof() )
	{
		...
		if ( maxln > 0 && max_row > (size_t)maxln )
			break;
...
bool wxCSVData::ReadFile( const wxString &file, int maxln )
{
	wxFFileInputStream in( file );
	if ( !in.IsOk() ) return false;
	return Read( in, maxln );
}
...
bool wxCSVData::ReadString( const wxString &data, int maxln )
{
	wxStringInputStream ss(data);
	return Read(ss, maxln);
}
